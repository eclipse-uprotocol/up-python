...
class TestRpcMapper(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_map_response_with_null_payload(self):
        ...
        with self.assertRaises(Exception) as exc_info:
            await RpcMapper.map_response(future_result, UUri)
        assert str(exc_info.exception) == f"Unknown payload. Expected [{UUri.__name__}]"
    ...
    async def test_map_response_with_ustatuserror_instance(self):
        # Test for: if isinstance(response, UStatusError): raise response
        status = UStatus(code=UCode.NOT_FOUND, message="Not found")
        error_instance = UStatusError(status)

        class RpcClientWithUStatusErrorInstance:
            async def invoke_method(self, uri, payload, options):
                return error_instance

        rpc_client = RpcClientWithUStatusErrorInstance()
        future_result = rpc_client.invoke_method(self.create_method_uri(), UPayload.EMPTY, None)

        with self.assertRaises(UStatusError) as exc_info:
            await RpcMapper.map_response(future_result, UUri)
        assert exc_info.exception.status == status

    async def test_map_response_with_general_exception_instance(self):
        # Test for: if isinstance(response, Exception): raise response
        exception_instance = ValueError("Some error")

        class RpcClientWithExceptionInstance:
            async def invoke_method(self, uri, payload, options):
                return exception_instance

        rpc_client = RpcClientWithExceptionInstance()
        future_result = rpc_client.invoke_method(self.create_method_uri(), UPayload.EMPTY, None)

        with self.assertRaises(ValueError) as exc_info:
            await RpcMapper.map_response(future_result, UUri)
        assert str(exc_info.exception) == "Some error"

    async def test_map_response_with_unexpected_type(self):
        # Test for final fallback TypeError
        class RpcClientWithUnexpectedType:
            async def invoke_method(self, uri, payload, options):
                return 42  # unexpected type: int

        rpc_client = RpcClientWithUnexpectedType()
        future_result = rpc_client.invoke_method(self.create_method_uri(), UPayload.EMPTY, None)

        with self.assertRaises(TypeError) as exc_info:
            await RpcMapper.map_response(future_result, UUri)

        assert "Expected response of type" in str(exc_info.exception)
        assert "but got <class 'int'>" in str(exc_info.exception)

    async def test_map_response_to_result_with_ustatuserror_instance_returned(self):
        # Cover the case where the response itself is a UStatusError instance
        status = UStatus(code=UCode.PERMISSION_DENIED, message="Access denied")
        error_instance = UStatusError(status)

        class RpcClientReturningUStatusError:
            async def invoke_method(self, uri, payload, options):
                return error_instance

        rpc_client = RpcClientReturningUStatusError()
        future_result = rpc_client.invoke_method(self.create_method_uri(), UPayload.EMPTY, None)
        result = await RpcMapper.map_response_to_result(future_result, UUri)

        assert result.is_failure()
        assert result.failure_value().code == UCode.PERMISSION_DENIED
        assert result.failure_value().message == "Access denied"
